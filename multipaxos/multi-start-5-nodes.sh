...
# MultiPaxos 5-node helper: start | stop | status | restart | clean
BIN="bin/multipaxos_replicant"
LOGDIR="logs"
NODES=(0 1 2 3 4)

check_bin() {
  [ -x "$BIN" ] || { echo "Error: $BIN not found or not executable."; echo "Build with: go build -o $BIN ./main/main.go"; exit 1; }
}
...
start_one() {
  ...
  mkdir -p "$LOGDIR"
  "$BIN" -id "$i" -c "config/config_node$i.json" -d >"$LOGDIR/node_$i.log" 2>&1 &
